...
class WikiCacher:
    ...
    def visit(self, visiting_page, depth):
        if len(self.pages) > self.pages_limit or depth > self.depth_limit or len(self.pages) > self.depth_limit:
            return

        current_page = visiting_page.removeprefix(self.domain)

        logging.info(f"Visiting page: {current_page} at depth: {depth}")
    ...
    def cache(self):
        ...
        # Проверка на слишком малое количество страниц после завершения обработки
        if (len(self.pages) < self.min_pages) and ():
            logging.warning("The result is too small (<20 pages). Check depth limit parameter or consider choosing another start page.")

    def save_json(self):
        graph = networkx.Graph()

        graph.add_edges_from(self.edges)
        json.dump(networkx.node_link_data(graph), open('wiki.json', 'w'))
