...
Handlers:
    ...
    uniform_select - The handler selects the subject assasin's uniform.
    mantle_select - The handler selects the subject cherubim mantle.
    ...
    human_select - The handler sets the character type Human.
    warrior_select - The handler sets the character type warrior.
    knight_select - The handler sets the character type knight.
    hunter_select - The handler sets the character type hunter.
...
def get_actions(
    ...
    if current_location is not None:  # Check that we have correct location
        ...
        if (current_location.id == 18) and ("E-key" in current_protagonist.inventory):
            list_of_actions = '🗽  <b>ESCAPE</b> - to escape from "Calanthor - a land of eternal summer and magical forests"  🗽\n' + list_of_actions
...
async def escape(message: Message):
    ...
    await message.answer('CONGRATULATIONS, YOU HAVE <s>LOS...</s> ESCAPED FROM <b>"Calanthor - a land of eternal summer and magical forests"</b>!')
    await sleep(1)
    await message.answer("The whole story turned out to be real, it turns out that you saved someones life.")
    await message.answer("If you have any comments or suggestions, please write to the author at <b>@puncherwc</b>.")
    await sleep(3)
    await message.answer("<b> THANK YOU!</b>")

    buttons = [
        [KeyboardButton(text="🗡SET ITEM🗡"), KeyboardButton(text="🎭SET TYPE🎭")],
        [KeyboardButton(text="👹START GAME👹"), KeyboardButton(text="🌚EXIT🌚")]
    ]
    ...
    await message.answer(
        ...
        '3️⃣  START GAME - to immerse yourself in "Calanthor - a land of eternal summer and magical forests"\n' +
...
async def pass_task(message: Message):
    ...
    if main_location.id == 2:
        ...
        await message.answer('You get <b>"Dwarf Gun"</b>!')

        main_protagonist.inventory.append("Dwarf Gun")
...
async def end(message: Message):
    ...
    await message.answer(f'Hello new hero, <b>{message.from_user.first_name}</b>!' +
                         f'\nWelcome to the game 🏃‍♂️<tg-spoiler>️"Escape From Calanthor - a land of eternal summer and magical forests"</tg-spoiler> 🌐\n')
    await message.answer("<u>PLEASE, CHOICE THE BUTTON AND PRESS IT</u>:\n\n" +
                         '1️⃣  TEST - to get to know the "Calanthor"\n' +
...
async def start(message: Message):
    ...
    await message.answer(f'Hello new hero, <b>{message.from_user.first_name}</b>!' +
                         f'\nWelcome to the game 🏃‍♂️<tg-spoiler>️"Escape From Calanthor"</tg-spoiler> 🌐\n')
    await message.answer("<u>PLEASE, CHOICE THE BUTTON AND PRESS IT</u>:\n\n" +
                         '1️⃣  TEST - to get to know the "Calanthor"\n' +
                         "2️⃣  SET UP - to customize your character, start the adventure\n",
                         reply_markup=keyboard)
...
async def give_up(message: Message):
    ...
    await message.answer(
        ...
        "1️⃣  SET ITEM - to choice a protagonist item\n" +
        "2️⃣  SET TYPE - to choice a protagonist type\n" +
        '3️⃣  START GAME - to immerse yourself in "Calanthor"\n' +
        "4️⃣  EXIT - to exit from settings",
        reply_markup=keyboard
    )
...
@dispatcher.message(F.text == "🥷🏻ASSASIN'S UNIFORM🥷🏻")
async def uniform_select(message: Message):
    """The handler to choice item assasins uniform.
...
@dispatcher.message(F.text == "🥼CHERUBIM MANTLE🥼")
async def mantle_select(message: Message):
    """The handler to choice item cyber Cherubim Mantle.
...
@dispatcher.message(F.text == "🧤CHIEFTAIN'S GLOVES🧤")
async def gloves_select(message: Message):
    """The handler to choice item Chieftain's Gloves.
...
async def set_item(message: Message):
    ...
    buttons = [
        [KeyboardButton(text="🧤CHIEFTAIN'S GLOVES🧤"), KeyboardButton(text="🥼CHERUBIM MANTLE🥼")],
        [KeyboardButton(text="🥷🏻ASSASIN'S UNIFORM🥷🏻"), KeyboardButton(text="🌊THE SEA CHAIN🌊")],
    ...
    await message.answer(
        ...
        '1️⃣  CHIEFTAIN\'S GLOVES - to choice the character item Chieftain\'s Gloves\n' +
        '2️⃣  CHERUBIM MANTLE - to choice the character item "Cherubim Mantle"\n' +
        '3️⃣  ASSASIN\'S UNIFORM  - to choice the character item "Assasin\'s Uniform "\n' +
...
async def set_type(message: Message):
    ...
    buttons = [
        [KeyboardButton(text="🧔🏻HUMAN🧔🏻"), KeyboardButton(text="🤺WARRIOR🤺")],
        [KeyboardButton(text="🏇🏼KNIGHT🏇🏼"), KeyboardButton(text="🕵🏼HUNTER🕵🏼")],
        [KeyboardButton(text="🚶‍♂️BACK🚶‍♂️"), ]
    ]
    keyboard = ReplyKeyboardMarkup(keyboard=buttons, resize_keyboard=True)
    ...
    await message.answer(
        ...
        '1️⃣  HUMAN - to set the character type "Human"\n' +
        '2️⃣  WARRIOR - to set the character type "Warrior"\n' +
        '3️⃣  KNIGHT  - to set the character type "Knight"\n' +
        '4️⃣  HUNTER - to set the character type "Hunter"' +
        '5️⃣  BACK - to return back to set up',
        reply_markup=keyboard
    )


@dispatcher.message(F.text == "🧔🏻HUMAN🧔🏻")
async def human_select(message: Message):
    """The handler to choice type human.
...
@dispatcher.message(F.text == "🤺WARRIOR🤺")
async def warrior_select(message: Message):
    """The handler to choice type warrior.
...
@dispatcher.message(F.text == "🏇🏼KNIGHT🏇🏼")
async def knight_select(message: Message):
    """The handler to choice type knight.
...
@dispatcher.message(F.text == "🕵🏼HUNTER🕵🏼")
async def hunter_select(message: Message):
    """The handler to choice type hunter.
...
@dispatcher.message(F.text == "👹START GAME👹")
