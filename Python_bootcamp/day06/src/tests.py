...
if __name__ == "__main__":
    # Test cases:
    number = 1
    ex00_command_one = "python3 reporting_client.py"
    ...
    number = 2
    ex00_command_two = "python3 reporting_client.py 1"

    check_command(ex00_command_two, ex00_expected_output_one, number)

    number = 3
    ex00_command_three = "python3 reporting_client.py 1 zxc"
    ex00_expected_output_three = "Error converting 'zxc' to float: could not convert string to float: 'zxc'\nVALUE ERROR: could not convert string to float: 'zxc'"


    check_command(ex00_command_three, ex00_expected_output_three, number)

    number = 4
    ex00_command_four = "python3 reporting_client.py zxc 1"

    check_command(ex00_command_four, ex00_expected_output_three, number)

    number = 5
    ex00_command_five = "python3 reporting_client.py -666 10"
    ex00_expected_output_five = """All coordinates: [-666.0, 10.0]
ERROR! INCORRECT COORDINATES."""

    check_command(ex00_command_five, ex00_expected_output_five, number)

    number = 6
    ex00_command_six = "python3 reporting_client.py -666 10"

    check_command(ex00_command_six, ex00_expected_output_five, number)

    number = 7
    ex00_command_seven = "python3 reporting_client.py -666 10"

    check_command(ex00_command_seven, ex00_expected_output_five, number)

    for i in range(25):
        ex01_command_one = "python3 reporting_client_v2.py 45 45"

        subprocess.run(ex01_command_one, text=True, shell=True)

    for i in range(25):
        ex02_command_one = "python3 reporting_client_v3.py 45 45"

        subprocess.run(ex02_command_one, text=True, shell=True)

    ex02_command_two = "python3 reporting_client_v3.py scan 5 5"

    subprocess.run(ex02_command_two, text=True, shell=True)
    subprocess.run("python3 reporting_client_v3.py list_traitors", text=True, shell=True)
