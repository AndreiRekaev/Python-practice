...
def draw_game(stdscr, state: State) -> None:
    stdscr.clear()
    ...
    stdscr.addstr(0, 22, f"Score: {state.score}")
    stdscr.addstr(1, 22, f"High Score: {state.high_score}")
    stdscr.addstr(2, 22, f"Level: {state.level}")
    stdscr.addstr(3, 22, f"Speed: {state.speed}")
    stdscr.addstr(4, 22, f"Pause: {'Yes' if state.pause else 'No'}")
    stdscr.refresh()
...
def main(stdscr):
    ...
    key_action_map = {
        ord('q'): Action.TERMINATE.value,
        ord('a'): Action.LEFT.value,
        ord('d'): Action.RIGHT.value,
        ord('w'): Action.UP.value,
        ord('s'): Action.DOWN.value,
        ord('p'): Action.PAUSE.value,
        ord(' '): Action.START.value
    }

    while True:
        key = stdscr.getch()
        if key in key_action_map:
            user_input(key_action_map[key], True)
